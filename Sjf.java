...
public class Sjf {
	...
	public void updateCurrentProcess()
	{
		...
		{
			int min=Integer.MAX_VALUE,j=0,v=0;
			for(int i=0;i<ready_processes.size();i++)
			{
				
				if(processes.get(ready_processes.get(i)).C_remaining<min)
				{
					v=ready_processes.get(i);
					min=processes.get(v).C_remaining;
					j=i;
				}
			}
			current_process=v;
			ready_processes.remove(j);
