...
public class RoundRobin {
	...
	no_processes,terminated,used_cycles,io_cycles,rr_cycles;
	...
	public void printProcessesState()
	{
		...
		for(int i=0;i<no_processes;i++)
		{
			...
			if(p.state.equals("ready")||p.state.equals("unstarted")
					||p.state.equals("terminated"))
			...
			if(p.state.equals("running"))
				System.out.print("\t"+Integer.toString(rr_cycles)+".\t");
		}
	...
	public void updateCurrentProcess()
	{
		if (ready_processes.size()>0) 
		{
			...
			if(processes.get(current_process).burst_cycles==0)
				current_process=-1;
	...
	public void updateStateCurrentProcess(int x)
	{
		...
		else if (p.state.equals("running"))
		{
			...
			p.burst_cycles-=1;
			p.C_remaining-=1;
			if ((rr_cycles==0)&&(p.burst_cycles>0))
			{
				p.state="ready";
				ready_processes.add(x);
			}
			else if(p.burst_cycles==0)
			{
	...
	public void roundrobin()
	{
		...
		for(int t=0;t<Integer.MAX_VALUE;t++)
		{
			...
			if(current_process==-1)
			{
				...
				if(current_process!=-1)
				{
					if(processes.get(current_process).burst_cycles>=2)
						rr_cycles=2;
					else
						rr_cycles=1;
				}
			}
			else if((rr_cycles==0)||processes.get(current_process).state!="running")
			{
				updateCurrentProcess();
				if(current_process!=-1)
				{
